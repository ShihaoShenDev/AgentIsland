...
## 更新摘要
**变更内容**
- 移除了对已删除的包管理与锁定文件策略文档（_module.yaml 和 dependency_management.md）的引用
- 更新了依赖管理章节，反映当前基于 .csproj 的 NuGet 包管理方式
- 简化了包版本管理说明，聚焦于实际的实现模式
...
AgentIslandJsonContext --> EchoCaveResult : "序列化"
AgentIslandJsonContext --> ComponentTextListResult : "序列化"
AgentIslandJsonContext --> ComponentTextEntry : "序列化"
...
AgentIslandJsonContext --> List_ComponentTextEntry : "序列化"
```

图表来源
- [Models/AgentIslandJsonContext.cs:1-24](file://Models/AgentIslandJsonContext.cs#L1-L24)
...
- 包管理与版本
  ...
  - 使用变量集中管理 ClassIsland.PluginSdk 版本，便于统一升级。
  - 通过 PackageReference 明确声明依赖包及其版本，支持 NuGet 包的自动还原。
- 插件清单
  - manifest.yml 定义插件 ID、入口程序集、API 版本与平台支持。

```mermaid
graph TB
P["AgentIsland.csproj"] --> S["Sentry 包 (v5.14.1)"]
P --> MCP["DotNetCampus.ModelContextProtocol 包 (v0.1.0-alpha.40)"]
P --> ACP["AgentClientProtocol 包 (v0.1.5)"]
P --> CISDK["ClassIsland.PluginSdk 包 (v1.7.106.2-dev-v2)"]
M["manifest.yml"] --> E["entranceAssembly=AgentIsland.dll"]
M --> API["apiVersion=2.0.0.0"]
```
...
- 包版本与兼容性
  ...
  - 使用 MSBuild 变量统一管理 SDK 版本，便于批量升级。
- 清单与分发
  - manifest.yml 中的 apiVersion 应与宿主 API 对齐，确保插件在不同宿主版本间的兼容性。
  - 通过 dependencies 字段声明可选依赖，支持插件间的组合使用。
