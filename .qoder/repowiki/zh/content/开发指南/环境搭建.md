...
- 构建脚本 build-debug.ps1 与 build-release.ps1 的参数与执行步骤
- 项目文件结构与清单 manifest.yml 字段含义
- 常见环境问题排查与解决方案

## 项目结构
AgentIsland 是一个基于 Avalonia 的 Windows 桌面插件，目标框架为 net8.0-windows，通过 ClassIsland.PluginSdk 集成到 ClassIsland 宿主中，并在运行时暴露 MCP Server 给外部智能体或工具调用。

```mermaid
graph TB
A["AgentIsland.csproj<br/>项目定义与依赖"] --> B["Plugin.cs<br/>插件入口与生命周期"]
B --> C["Mcp/McpServerManager.cs<br/>MCP 服务器管理"]
C --> D["Mcp/Tools/*<br/>工具实现"]
B --> E["Models/AgentIslandSettings.cs<br/>设置模型"]
B --> F["Services/SentryTelemetryService.cs<br/>遥测服务"]
A --> G["manifest.yml<br/>插件清单"]
```
...
- [AgentIslandSettings.cs:28-173](file://Models/AgentIslandSettings.cs#L28-L173)
...
### MCP 服务器管理器（Mcp/McpServerManager.cs）
- 职责
...
```mermaid
graph LR
P["AgentIsland.csproj"] --> Sdk["ClassIsland.PluginSdk"]
...
- [AGENTS.md:20-23](file://AGENTS.md#L20-L23)

### 二、IDE 配置要点
- Visual Studio
  - 打开项目后，确保工作负载包含“.NET 桌面开发”和“ASP.NET 和 Web 开发”（以便调试 HTTP 服务）。
...
- build-debug.ps1
  ...
  - 行为：终止现有 ClassIsland 进程 -> dotnet build -> 若成功则启动 ClassIsland 并传入调试输出目录。
  - 适用场景：日常开发与联调。
- build-release.ps1
  - 用途：构建发布版本并启动 ClassIsland 调试实例，便于验证打包产物。
  - 行为：终止现有 ClassIsland 进程 -> dotnet publish -c Release -> 若成功则启动 ClassIsland 并传入发布输出目录。
  - 适用场景：发布前验证与打包测试。
...
- name：插件名称
- description：插件描述
- entranceAssembly：插件入口程序集名
- url：仓库或主页链接
- version：插件版本
- apiVersion：适配的 ClassIsland API 版本
- author：作者
...
章节来源
- [AgentIslandSettings.cs:175-211](file://Models/AgentIslandSettings.cs#L175-L211)
