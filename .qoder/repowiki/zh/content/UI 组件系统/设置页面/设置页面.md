...
## 更新摘要
**所做更改**   
- 新增概览设置页面章节，详细说明 OverviewSettingsPage 的功能和实现
- 更新插件注册系统部分，说明新增的概览页面注册机制
- 完善设置页面架构图，包含概览页面
- 更新新设置页面开发指南，提供概览页面作为参考示例
...
- 详解各设置页面功能：概览页面、MCP 服务器配置、ACP 自动化设置、AI 文字组件配置、遥测监控设置
...
subgraph "设置页视图"
OverviewV["OverviewSettingsPage.axaml(.cs)<br/>概览页面"]
...
+OnInitialized()
+UpdateSystemStatus()
+OnOpenLink(object, RoutedEventArgs)
}
...
OverviewSettingsPage --> AgentIslandSettings : "读取状态"
```
...
- [SentryTelemetryService.cs:11-182](file://Services/SentryTelemetryService.cs#L11-L182)
- [OverviewSettingsPage.axaml.cs:18-55](file://Views/SettingsPages/OverviewSettingsPage.axaml.cs#L18-L55)

## 详细组件分析

### 概览设置页面
- 功能要点
  - 显示插件基本信息（名称、版本、作者、描述）
  - 展示核心功能概览（MCP 服务器、ACP 面板、AI 文字、遥测）
  - 提供快速链接（GitHub 仓库、问题反馈）
  - 实时显示系统状态（MCP 服务器状态、ACP 面板状态）
- 生命周期管理
  - 页面初始化时调用 UpdateSystemStatus() 获取当前状态
  - 直接访问 Plugin.Settings 静态属性获取设置信息
  - 支持外部链接打开功能
- 用户界面设计
  - 使用 FluentAvalonia 的 SettingsExpander 组件组织内容
  - 采用图标文本组合提升用户体验
  - 响应式布局支持滚动查看
...
sequenceDiagram
participant U as "用户"
participant V as "OverviewSettingsPage"
participant S as "Plugin.Settings"
participant OS as "操作系统"
U->>V : 打开概览页面
V->>V : OnInitialized()
V->>V : UpdateSystemStatus()
V->>S : 读取 IsEnabled, Port, IsAcpEnabled
V->>V : 更新 McpStatusText, AcpStatusText
U->>V : 点击 GitHub 链接
V->>OS : Process.Start(url)
```
...
- [OverviewSettingsPage.axaml:12-60](file://Views/SettingsPages/OverviewSettingsPage.axaml#L12-L60)
...
Plugin --> Overview["OverviewSettingsPage"]
...
图表来源
- [Plugin.cs:45-49](file://Plugin.cs#L45-L49)
- [OverviewSettingsPage.axaml.cs:33](file://Views/SettingsPages/OverviewSettingsPage.axaml.cs#L33)
- [AgentIslandSettings.cs:13-32](file://Models/AgentIslandSettings.cs#L13-L32)
- [SentryTelemetryService.cs:21-25](file://Services/SentryTelemetryService.cs#L21-L25)
...
- 概览页面状态显示不正确
  - 检查 Plugin.Settings 静态属性是否正确初始化
  - 确认 UpdateSystemStatus() 方法是否正常执行
  - 参考路径：[OverviewSettingsPage.axaml.cs:31-42](file://Views/SettingsPages/OverviewSettingsPage.axaml.cs#L31-L42)
- 遥测未上报
...
## 结论
设置页面系统以全局设置为中心，结合 MVVM 与集合变更通知，实现了简洁而可扩展的配置体验。新增的概览页面为用户提供了统一的入口和信息展示。遥测服务与隐私策略解耦清晰，支持运行时动态启停。各页面职责单一、交互明确，便于后续扩展与维护。
...
- [OverviewSettingsPage.axaml.cs:25-29](file://Views/SettingsPages/OverviewSettingsPage.axaml.cs#L25-L29)
- [Plugin.cs:29-53](file://Plugin.cs#L29-L53)
...
- 最佳实践
  ...
  - 避免在属性变更回调中执行耗时操作
- 概览页面参考实现
  - 直接使用 Plugin.Settings 静态属性获取设置信息
  - 在 OnInitialized 中调用状态更新方法
  - 使用 SettingsExpander 组件组织内容结构
  - 支持外部链接打开功能
...
### 插件注册系统增强
- 新增概览页面注册：通过 services.AddSettingsPage<Views.SettingsPages.OverviewSettingsPage>() 集成到 ClassIsland 设置界面
- 页面分类：概览页面使用 SettingsPageCategory.External 分类，与其他设置页面保持一致
- 页面标识：使用唯一的 id "agentisland.overview" 和友好的 name "AgentIsland / 概览"
- 注册顺序：概览页面优先注册，作为用户进入设置界面的第一个页面

章节来源
- [Plugin.cs:45](file://Plugin.cs#L45)
- [OverviewSettingsPage.axaml.cs:13-17](file://Views/SettingsPages/OverviewSettingsPage.axaml.cs#L13-L17)